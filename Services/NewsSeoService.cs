...
public static class NewsSeoService
{
  ...
  private static string BuildHomeLatestItemHtml(NewsItem item)
  {
    ...
    var imageUrl = hasImage ? item.ImageUrl! : "/public/images/index/news1.png";
    var imageHtml = $"<img src=\"{Encode(imageUrl)}\" alt=\"\" loading=\"lazy\" decoding=\"async\">";

    return $"""
            <a class="home-latest__item" href="{link}">
              <span class="home-latest__media {(hasImage ? "has-image" : "is-default")}">{imageHtml}</span>
              <span class="home-latest__body">
                <span class="home-latest__meta"><time class="home-latest__date" datetime="{Encode(item.Date)}">{Encode(item.Date)}</time><span class="home-latest__category">{tag}</span></span>
                <span class="home-latest__title-row"><strong>{Encode(item.Title)}</strong><span class="home-latest__more" aria-hidden="true"><svg class="home-latest__arrow-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"></path></svg></span></span>
              </span>
            </a>
            """;
  }
