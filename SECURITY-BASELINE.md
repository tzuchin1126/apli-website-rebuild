...
目前附件限制為 PDF、Word、Excel，最大 10 MB；圖片原始檔在後台限制為 5 MB，會縮至最長邊 1920px 並轉成 WebP，後端只接受 1 MB 內的儲存檔。圖片與附件都會檢查檔案內容簽章，不只相信使用者提供的副檔名。
...
上傳檔案使用隨機 GUID 檔名儲存，下載端點會拒絕包含路徑的檔名；附件存放於 `App_Data/news/`，壓縮圖片存放於 `App_Data/news/images/`，兩者都不直接位於公開靜態目錄。
