...
function initCertificationPreview() {
  ...
  function getPages(decade) {
    ...
    const decadeIndex = decades.findIndex((item) => item.start === decade.start);
    let nextDecadeIndex = decadeIndex + 1;
    while (entries.length % pageSize !== 0 && nextDecadeIndex < decades.length) {
      const remaining = pageSize - (entries.length % pageSize);
      entries.push(...decades[nextDecadeIndex].years.slice(0, remaining));
      nextDecadeIndex += 1;
    }
    const pages = [];
