...
## 已完成
### 2026-09-03 羽球隊歷年成績次標題與項目符號調整

- 將「歷年成績殊榮」維持為 `h3` 次標題並套用精簡字級。
- 將 `.affiliate-panel li::marker` 改為小方形項目符號，保留原有橘色重點色與成績內容。
- 實際驗證：Edge Desktop `1432px` 確認「歷年成績殊榮」為 `h3`、字級 `20px`，清單為 `square` marker 且 marker 字級約 `12.6px`；Mobile `390px` 確認字級與無水平溢出。未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏羽球隊套用會舘雙欄輪播版型

- 將亞柏羽球隊改為參考亞柏會舘的雙欄內容排版：左側隊名與介紹文字，右側單張賽場圖片輪播及 3 段進度線。
- 將原有三張羽球隊圖片接入既有 `data-affiliate-carousel` 邏輯，保留歷年成績殊榮、既有文字內容與資料；手機版沿用單欄堆疊。
- 實際驗證：Edge Desktop `1432px` 確認左側隊名與介紹、右側單張輪播及 3 段進度線；點擊第 3 段確認圖片切換，Mobile `390px` 確認單欄排列、歷年成績內容仍存在且無水平溢出。未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏會舘標題層級整理

- 移除亞柏會舘專用 `affiliate-tagline`，保留「亞柏會舘」主標題與介紹內容。
- 將 `affiliate-club-cta-title` 維持為語意次標題 `h3`，並套用精簡次標題字級；未影響其他企業頁面的 tagline。
- 實際驗證：Edge Desktop `1432px` 確認會舘 tagline 已移除，`affiliate-club-cta-title` 為 `h3` 且字級為 `20px`；Mobile `390px` 確認無水平溢出。未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏會舘套用油品雙欄輪播版型

- 將亞柏會舘改為與亞柏油品一致的雙欄內容排版：左側 tagline、標題與介紹文字，右側單張環境圖片輪播及進度線。
- 將原有三張會舘圖片接入既有 `data-affiliate-carousel` 邏輯，保留聯絡資訊、外部網站連結與既有資料內容；手機版沿用單欄堆疊與既有 responsive 行為。
- 實際驗證：Edge Desktop `1432px` 確認左側介紹、右側單張輪播與 3 段進度線；點擊第 2 段確認圖片切換，Mobile `390px` 確認單欄排列與無水平溢出。未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 收斂仁武與八卦寮 marker 引導線距離

- 延長仁武站與八卦寮站右側 marker 引導線至 `72px`，讓線段更接近圓點；保留兩站文字位置、其他站點設定與既有互動邏輯。
- 實際驗證：Edge Desktop `1432px` 確認仁武與八卦寮引導線寬度為 `72px`、線段距離圓點約 `3px`，且頁面無水平溢出；未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 服務據點仁武與八卦寮 marker 標籤調整

- 將仁武站標籤移至地圖右側、八卦寮站標籤下方；僅將八卦寮站標籤外移，避免文字直接壓在地圖上，其他站點標籤位置維持不變。
- 將所有 marker 引導線由品牌橘色改為與 marker 文字相同的正文灰色；保留站點座標、文字內容與既有點擊切換邏輯。
- 實際驗證：Edge Desktop `1432px` 確認仁武站標籤位於八卦寮站下方且兩者皆在地圖右側之外，引導線為正文灰色；Tablet `1024px` 與 Mobile `390px` 確認無水平溢出，Mobile 維持隱藏 marker 標籤。未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 統一服務據點 marker 標籤背景

- 移除所有服務據點 marker 文字標籤的白色背景，統一使用透明背景，避免仁武與八卦寮標籤壓在地圖色塊上時看起來與其他站點不同；保留文字、引導線與點擊功能。
- 實際驗證：Edge Desktop 確認 marker 標籤計算背景為透明且地圖文字仍可讀；未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 服務據點地圖與站點標示整理

- 將「八卦寮」marker、地圖標籤與右側站點名稱統一為「八卦寮站」，使其與其他站點顯示一致。
- 將服務據點地圖列改為上對齊，並收斂 SVG 上方內部留白造成的視覺間距，讓地圖內容位於標題說明下方；保留右側站點資訊與既有互動邏輯。
- 實際驗證：Edge Desktop `1432×900` 確認標題說明下方直接呈現地圖與資訊區；`1024×768`、`768×1024`、`390×844` 確認響應式版面無水平溢出；八卦寮名稱與標籤一致。
- `node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check` 均通過；未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏油品地圖放大與南部站點避讓

- 將 Desktop 地圖欄位放大，視覺寬度上限調整為 `620px`；在目前容器下實際約 `580px`，1024px 約 `380px`，768px 約 `520px`，Mobile 依容器寬度自適應。
- 將路竹 marker 移近仁武／八卦寮，並調整小港 marker 與標籤位置，避免仁武站被小港站遮擋；保留地圖 SVG、站點資料與既有 JavaScript 互動邏輯。
- 實際驗證：Edge Desktop、Tablet `1024×768`、Small Tablet `768×1024`、Mobile `390×844` 確認無水平溢出；路竹、仁武、八卦寮、小港四個受影響站點均可切換至正確資訊，Desktop 畫面確認地圖放大與標籤避讓。
- `node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`、`git diff --check`；未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 再次修正新屋 marker 對位

- 將新屋 marker 由 `top: 37% / left: 70%` 微調為 `top: 35.5% / left: 70.6%`，對齊桃園橘色行政區內部；未修改地圖 SVG、站點資料或互動邏輯。
- 實際驗證：未執行，依使用者要求不進行驗證。
- 未建立 commit。

### 2026-09-03 修正新屋與仁武地圖 marker 對位

- 微調新屋與仁武 marker 的 CSS 百分比座標，讓兩個站點圓點落在對應的橘色行政區內；同時拉開仁武與八卦寮的命中距離。
- 保留地圖 SVG、站點名稱、橘色區域與既有 JavaScript 點擊切換邏輯，未修改站點資料。
- 實際驗證：Edge Desktop `1432×900` 截圖確認新屋與仁武圓點位於橘色區域；Desktop、Tablet `1024×768`、Mobile `390×844` 確認 marker 不溢出且無水平溢出；逐一點擊 9 個 marker，皆切換至對應站點資訊。
- `node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check` 均通過；未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏油品服務據點版面與營業時間欄位調整

- 收斂「服務據點」說明文字與地圖的垂直間距，並將服務據點區塊上方內距調整為較緊湊的響應式值；Desktop 地圖視覺寬度控制為 `420px`，右側資訊維持左右排列。
- 1024–1199px 保留雙欄並縮放為 `320px / 340px`，768px 以下改為地圖在上、資訊在下；未改變左側關係企業選單、地圖 marker／橘色區域、點擊切換邏輯或既有資料結構。
- 右側站點資訊改為站名、地址、電話、營業時間的垂直層級；9 個站點補上 `data-business-hours` 資料，版型以既有卡片資料同步顯示營業時間，未虛構其他欄位資料。
- 實際驗證：Edge `1432×900` 確認地圖視覺寬度 `420px`、說明至地圖間距 `24px`、資訊欄位垂直排列；`1024×768` 確認雙欄與無水平溢出；`768×1024`、`390×844` 確認單欄順序與無水平溢出；點擊新園站確認站名、地址、電話、營業時間同步更新，9 個站點營業時間屬性與畫面值全部一致。
- 逐一點擊 marker 時確認既有「仁武站／八卦寮」按鈕的重疊命中問題仍存在；本次依照保留既有 marker 座標與互動邏輯的範圍未處理該既有問題。
- `node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check` 均通過。
- 未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 移除亞柏油品企業理念灰框底部橘線

- 移除亞柏油品「企業理念」灰色引用框底部的品牌橘色邊線，保留灰色背景、文字、引號裝飾與其他區塊邊線。
- 更新 `affiliates.css` cache key，確保實際頁面載入本次樣式。
- 實際驗證：Edge `1432×900` 與 `390×844` 確認 `.affiliate-oil-quote` 的 `border-bottom-width` 為 `0px`、灰色背景仍存在且無水平溢出；`node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check` 均通過。
- 未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏油品地圖路竹站標籤避讓

- 將「路竹站」地圖標籤向左錯開並延長專屬引導線，避免與「仁武站」文字框重疊；保留兩站圓點座標、點擊切換與其他站點設定不變。
- 更新 `affiliates.css` cache key，確保實際頁面載入本次樣式。
- 實際驗證：Edge `1432×900` 與 `1024×768` 確認路竹站／仁武站文字框實際交集面積為 `0`、路竹引導線仍連向站點標記；`390×844` 確認 9 個地圖標籤依原規則隱藏且無水平溢出；`node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check` 均通過。
- 未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 亞柏油品標題字級統一

- 將亞柏油品的「企業理念」與「服務據點」標題，統一套用世新貨櫃「服務項目」使用的 `--font-size-card-title-compact` 與對應行高；未修改其他企業或全站標題。
- 更新 `affiliates.css` cache key，確保實際頁面載入本次樣式。
- 實際驗證：Edge `1432px` 確認三個標題皆為 `20px`、`30px` 行高；`390×844` 確認手機版無水平溢出；`node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check` 均通過。
- 未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。

### 2026-09-03 Affiliates 筆電內容寬度與邊距修正

- 僅在 `1024–1439px` 筆電範圍將關係企業內容容器收窄為最大 `1200px`、最小水平內距 `32px`，並取消左側企業導覽向左 `-40px` 外推；桌機與手機版規則維持不變。
- 更新 `affiliates.css` cache key，確保實際頁面載入本次樣式。
- 實際驗證：Edge `1432px` 確認內容寬度 `1200px`、左右約 `116px`；`1024×768` 確認內容寬度 `960px`、左右 `32px`；`1440×900` 維持原 `1324px` 桌機容器；`390×844` 維持手機容器與水平 Tabs；各尺寸均確認無水平溢出且瀏覽器無錯誤。另完成 `node --check wwwroot/js/pages/affiliates.js`、`dotnet build -c Release --no-restore`（0 warnings、0 errors）、`git diff --check`。
- 未驗證項目：實體裝置、其他瀏覽器與人工無障礙驗收。
- 未建立 commit。
