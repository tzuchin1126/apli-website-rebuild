...
## 待處理事項
- [x] 2026-08-29 移除首頁 Hero 圖片的 `filter: saturate(.88) contrast(.96)` 色彩／對比調整，恢復圖片原始色彩呈現；保留 Hero 黑色可讀性遮罩、文字陰影、裁切、輪播與動畫行為，並更新首頁 CSS cache key。已完成來源檢查與 `git diff --check`，未進行瀏覽器／實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
- [x] 2026-08-28 恢復第二張 Services「貨櫃清潔與維修」圖片為最新 commit 的未調色原圖：同步恢復首頁圖片實際尺寸 metadata `4912×3264`；第一張與第三張未修改。已完成圖片目視、格式／尺寸與 `git diff --check` 檢查，尚未 commit。
- [x] 2026-08-28 重新調整三張 Services 服務圖片影像風格：保留原本物流作業場景、設備、人物、貨櫃、視角與主要構圖，改為自然中性白平衡、明亮通透、適度控制高光、保留陰影細節與中等對比的高階企業商業攝影質感；維持自然藍天、紅褐／紅色貨櫃、藍色貨櫃、黃色與紅色機具的豐富但不過度鮮豔色彩，未加入冷暖濾鏡、灰霧、HDR、teal & orange、復古或 AI 風格。未新增或移除人物、設備、文字、Logo、Icon、按鈕或其他場景元素。已使用內建 ImageGen 編輯模式逐張生成，完成原圖與最終檔案目視確認、檔案格式／尺寸檢查與 `git diff --check`；未進行色彩儀器級量測、實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
- [x] 2026-08-28 Services Card 微調：保留現有版型、圖片尺寸、三欄與整張可點擊，文字區改為 `padding: 24px 24px 26px` 並移除額外最小高度；Card 本體不使用 `translateY`，圖片容器維持 `overflow: hidden`，Desktop Hover 收斂為圖片 `scale(1.03)`、`0.35s ease` 與邊框加深，不新增陰影／Overlay／CTA／Icon。工作區瀏覽器已驗證 1432×786 桌機三張約 384×426px、900×900 平板兩欄三張約 418×447px、390×844 手機單欄三張約 350×394px，均等高且無水平溢位；已完成 `git diff --check`，尚未 commit。未進行實機／跨瀏覽器／人工無障礙驗收。
- [x] 2026-08-28 修正首頁 Services Card 標題字體：`home-service-grid__title` 內服務名稱改用既有品牌 `--font-body`（Noto Sans TC），與 Services 內頁服務標題規範一致；保留原字級、字重、顏色與版面，尚未 commit。
- [x] 2026-08-28 移除首頁 Services Card CTA：移除每張卡片底部「了解更多」文字與箭頭，保留整張卡片可點擊、服務圖片／標題／簡介；文字區底部 padding 收斂為 24px，三張卡片維持等高。工作區瀏覽器已驗證 1432×786 桌機三欄（約 384×446px）、900×900 平板兩欄（約 418×471px）及 390×844 手機單欄（約 350×421px），均無水平溢位、CTA 數量為 0，外層連結範圍與卡片一致；桌機 hover 實測為圖片 `scale(1.04)`、`0.35s` transition、Card `translateY(-2px)` 與邊框加深，未加入 Overlay。已完成 `dotnet build -c Release`（0 警告／0 錯誤）與 `git diff --check`。未進行實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
- [x] 2026-08-28 重新設計首頁 OUR SERVICES／我們的服務卡片：依參考圖改為白色 1px 淡邊框／14px 圓角卡片，圖片上／左右內縮 14px 並使用 12px 圓角，圖片下方以獨立 flex 內容區承接原有服務名稱與完整敘述，CTA 改為「了解更多 →」並固定於卡片底部；Services 標題至卡片間距為 40px，保留品牌色／字體／圖片／文案，移除 Overlay 與 gradient。工作區瀏覽器已驗證 1432×786 桌機三欄（卡片約 384×462px、gap 24px）、900×900 平板兩欄（卡片約 418×487px）及 390×844 手機單欄（卡片約 350×440px），各尺寸均無水平溢位且 CTA 相對各卡片底部對齊；已完成 `dotnet build -c Release`（0 警告／0 錯誤）與 `git diff --check`。未進行實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
- [x] 2026-08-28 恢復首頁 Services 卡片至上一版圖片內 Overlay 樣式：保留原有圖片比例、圖片底部漸層、白色標題／說明、標題底線互動與右上角細線箭頭；僅撤回上一筆參考圖卡片改版，保留全域 Laptop 間距與平整 Services 背景。已完成來源差異檢查與 `git diff --check`，尚未 commit。
- [x] 2026-08-28 依參考圖重設首頁 Services 卡片：圖片置於白色卡片上方、文字移至圖片下方、加入細邊框／輕陰影／MORE 右箭頭與等高內容；保留整張卡片連結、既有圖片與文案，移除圖片文字 Overlay。工作區預覽已驗證 1432×786 桌機三張卡片等高約 260px、圖片比例約 1.48、Grid 內容寬 1200px、間距 24px，及 390×844 手機三張卡片等高約 237px；桌機／手機均無水平溢位。已完成 `dotnet build -c Release`（0 警告／0 錯誤）、`git diff --check` 與瀏覽器 DOM／computed 檢查；未進行實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
- [x] 2026-08-28 建立全域 Laptop viewport 間距規則：依目前瀏覽器 1432×786、DPR 2 與台泥首頁量測結果，1200–1440px 使用 1280px 內容外框與 40px 內容內距，並將 Header 內距同步為 40px；首頁、About、Services、Careers、公司沿革、聯絡、營運資源、職安、關係企業與 News 均完成全域套用。工作區預覽已驗證 1432×786（左右外側 76px、內側 40px）、1366px（左右外側 43px、內側 40px）、1280px（左右外側 24px、內側 40px）及 390×844 手機（維持左右 20px），各頁水平溢位為 0；台泥首頁同 viewport 外框為 1280px、左右 76px、內側 40px。已完成 CSS／cache key 檢查、桌機／手機瀏覽器檢查與 `git diff --check`；未進行實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
- [x] 2026-08-28 調整首頁 Services 背景為平整 `#f7f7f7` 淡灰色區塊：移除上方右高左低斜切與底部白色斜切層；未修改服務圖片、文字、Grid、Overlay、Hover 或 Icon。已完成 CSS 來源檢查與 `git diff --check`，尚未進行瀏覽器／實機／跨瀏覽器／人工無障礙驗收，尚未 commit。
